...
const MultiSelectOptionsList = require('./multi_select_options_list');

const MultiSelectKeyCodeMixin = require('./multi_select_key_code_mixin');
const MultiSelectArrow = require('./multi_select_arrow');
...
const MultiSelect = React.createClass({
  ...
  render: function() {
    return <span ref="multiSelectContainer" className="pt-multi-select" onKeyDown={this._handleKeyDown}>
  ...
  _preProcessOptions: function(options) {
    ...
    for (let i = 0; i < modifiedOptions.length; i++) {
  ...
  _handleOptionChange: function() {
    const newlyAddedOption = this.state.selectedOptions[this.state.selectedOptions.length - 1];
  ...
  _showPlaceholder: function(filterText) {
    if (filterText.length === 0 && this.state.selectedOptions.length === 0) return true;
    return false;
  },
